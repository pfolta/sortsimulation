...
describe("ArrayConstructor Utils", () => {
    describe("fewUnique", () => {
        ...
        it("returns an array of integers", () => {
            Array.fewUnique(size, uniqueCount).forEach((number) => expect(Number.isInteger(number)).toBe(true));
        });

        it("returns an array with the specified size", () => {
            expect(Array.fewUnique(size, uniqueCount)).toHaveLength(size);
        });

        it("returns an array with the specified number of unique elements", () => {
            expect([...new Set(Array.fewUnique(size, uniqueCount))]).toHaveLength(uniqueCount);
        });

        it("returns an array where every number is in [1, size]", () => {
            Array.fewUnique(size, uniqueCount).forEach((number) => {
