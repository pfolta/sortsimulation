...
describe("NumberArrayView", () => {
    ...
    it("renders correctly", () => {
        const component = renderWithProviders(<NumberArrayView array={array} colorMap={colorMap} />);
    ...
    it("applies the colorMap in the correct order", () => {
        const component = renderWithProviders(<NumberArrayView array={array} colorMap={colorMap} />);
        const elements = component.root.findAllByType(StyledNumberElement);

        expect(elements).toHaveLength(array.length);

        elements.forEach((element, index) => expect(element.props.color).toBe(colorMap[index]));
    });
});
